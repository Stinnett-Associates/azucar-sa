...
[cmdletbinding()]
    ...
    Process{
        ...
        Write-AuditorMessage -WriteLog $WriteLog -Message ($message.AuditorADGeneralTaskMessage -f "Virtual Machines", $bgRunspaceID, $PluginName, $AzureObject.TenantID) `
        ...
        if($Vms) {
            foreach($vm in $VMs){
                ...
                if($NetworkInterface){
                    Write-AuditorMessage -WriteLog $WriteLog -Message ("Get Network Interfaces for {0}..." -f $vm.name) `
                    ...
                    if($Result.name){
                        ...
                        if($PublicIPEndPoint){
                            ...
                            Write-AuditorMessage -WriteLog $WriteLog -Message ("Get Public IPAddress for {0}..." -f $vm.name) `
                                            -Plugin $PluginName -Verbosity $Verbosity -IsVerbose
    ...
    End{
        if($AllVMs){
            ...
            Write-AuditorMessage -WriteLog $WriteLog -Message ($message.AzureADGeneralQueryEmptyMessage -f "Virtual Machines", $AzureObject.TenantID) `
